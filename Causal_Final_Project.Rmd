...
Xi[,2] <- rnorm(n,50,5)# Age
Xi[,3] <- Xi[,2]/2.5 + Xi[,1] + rnorm(n) # BMI
Xi[,4] <- (rnorm(n,5,1)) # Tumor Size
Xi[,5] <- rbinom(10000, 1, (Xi[,3] - min(Xi[,3]))/35 ) # HBP
Xi[,6] <- rbinom(10000,1,(Xi[,3]-min(Xi[,3]))/(max(Xi[,3])-min(Xi[,3]))) # Relapse
...
#Add column names
colnames(X_mat) = c('beta0','Gender','Age', 'BMI', 'Size_Tumor','HBP', 'Relapse')

# Parameters 
# beta - vector of slopes to generate Y
beta <- c(-0.98, -0.803,1.17, 0.77, 0.027, 1.18, 0.187)
# vector of slopes to generate tau
gamma <- c(30, 1.64, -0.303, 0.0124, -1.523, 0.59, 0.8)
...
Y0_nl <- X_mat[,1]*-0.98 + X_mat[,2]*-0.803 + X_mat[,3]*1.17 + X_mat[,4]*log(seq(4,n+3, 1)) + X_mat[,5]*runif(n, 1, 20) + X_mat[,6]*1.18 + X_mat[,7]*runif(n, 0.1, 2)+ rnorm(n,0,1)

Y1_nl <- X_mat[,1]*-0.98 + X_mat[,2]*-0.803 + X_mat[,3]*1.17 + X_mat[,4]*log(seq(4,n+3, 1)) + X_mat[,5]*runif(n, 1, 20) + X_mat[,6]*1.18 + X_mat[,7]*runif(n, 0.1, 2)+ tau + rnorm(n,0,1)
```
...
q = ifelse(X_mat[,3] > 40 & X_mat[,4] < 18 & X_mat[,5] < 4 & X_mat[,7] == 0, -1, 1)
pi = pnorm(q)
treat = rbinom(n, 1, pi)
```
...
fit_lin <- lm(Y ~ Gender + Age + BMI + Size_Tumor + HBP + Relapse + treat,data = researcher_lin)
summary(fit_lin)
```

```{r}
# Linear Regression with non linear data
fit_nl <- lm(Y ~ Gender + Age + BMI + Size_Tumor + HBP + Relapse + treat,data = researcher_nl)
summary(fit_nl)
```
