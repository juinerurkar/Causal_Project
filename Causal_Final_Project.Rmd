...
Y0_nl <- X_mat[,1]*-0.98 + X_mat[,2]*-0.803 + X_mat[,3]*runif(n, 1, 5) + X_mat[,4]*1.17 + X_mat[,5]*0.77 + X_mat[,6]*runif(n, 0.02, 0.05) + X_mat[,7]*1.18 + X_mat[,8]*runif(n, 0.1, 0.2)+ rnorm(n,0,1)

Y1_nl <- X_mat[,1]*-0.98 + X_mat[,2]*-0.803 + X_mat[,3]*runif(n, 1, 5) + X_mat[,4]*1.17 + X_mat[,5]*0.77 + X_mat[,6]*runif(n, 0.02, 0.05) + X_mat[,7]*1.18 + X_mat[,8]*runif(n, 0.1, 0.2)+ tau + rnorm(n,0,1)
```
...
q = ifelse(X_mat[,2] < 170 & X_mat[,3] > 40 & X_mat[,5] < 4 & X_mat[,7] == 0, -1, 1)
pi = pnorm(q)
treat = rbinom(n, 1, pi)
```
...
fit_lin <- lm(Y ~ Gender + Weight + Age + BMI + Size_Tumor + HBP + Relapse + treat,data = researcher_lin)
summary(fit_lin)
```

```{r}
# Linear Regression with non linear data
fit_nl <- lm(Y ~ Gender + Weight + Age + BMI + Size_Tumor + HBP + Relapse + treat,data = researcher_nl)
summary(fit_nl)
```
...
fit_cf_lin <- causal_forest(X = train_lin[,1:7],Y = train_lin$Y, W = train_lin$treat)
cf.pred_lin <- predict(fit_cf_lin, test_lin[,1:7])

#Causal forest with non linear data
fit_cf_nl <- causal_forest(X = train_nl[,1:7],Y = train_nl$Y, W = train_nl$treat)
cf.pred_nl <- predict(fit_cf_nl, test_nl[,1:7])
```
...
fit_bayesian_lin <- bcf(researcher_lin$Y, researcher_lin$treat, as.matrix(researcher_lin[,1:7]), pihat = pi, nburn = 2000, nsim = 2000)

# Bayesian Causal Forest with non linear data
fit_bayesian_nl <- bcf(researcher_nl$Y, researcher_nl$treat, as.matrix(researcher_nl[,1:7]), pihat = pi, nburn = 2000, nsim = 2000)
```
...
hist(tauhat_nl, col = rgb(1,0,0, alpha = 0.7), ylim = c(0, 2000))
hist(tau, col = rgb(0,0,1, alpha = 0.7), add = T)
```
