...
Y0_nl <- 50 + ((X_mat$Weight^3) + (X_mat$Age^6) + exp(X_mat$Size_Tumor) + X_mat$Relapse) *(10^-9) + rbeta(n,0.5,1/3)

Y1_nl <- 50 + ((X_mat$Weight^3) + (X_mat$Age^6) + exp(X_mat$Size_Tumor) + X_mat$Relapse) *(10^-9) + rbeta(n,0.5,1/3)+ tau
```
...
mean_diff_lin_bcf = sqrt(mean((yhat_lin_bcf - researcher_lin$Y)^2))
print(mean_diff_lin_bcf)
mean_diff_lin_cf = sqrt(mean((cf.pred_lin$predictions - test_lin$Y)^2))
print(mean_diff_lin_cf)
mean_diff_lin_lr = sqrt(mean((Yhat_lin_lr - test_lin$Y)^2))
...
mean_diff_nl_bcf = sqrt(mean((yhat_nl_bcf - researcher_nl$Y)^2))
print(mean_diff_nl_bcf)
mean_diff_nl_cf = sqrt(mean((cf.pred_nl$predictions - test_nl$Y)^2))
print(mean_diff_nl_cf)
mean_diff_nl_lr = sqrt(mean((Yhat_nl_lr - test_nl$Y)^2))
print(mean_diff_nl_lr)
```

```{r}
plot(density(researcher_lin$Y - yhat_lin_bcf), main = "")
title(main = "Density - Residuals for BCF using linear data")
plot(density(test_lin$Y - cf.pred_lin$predictions ), main = "")
title(main = "Density - Residuals for CF using linear data")
plot(density(test_lin$Y - Yhat_lin_lr), main = "")
title(main = "Density - Residuals for Linear regression using linear data")
```

```{r}
plot(density(researcher_nl$Y - yhat_nl_bcf), main = "")
title(main = "Density - Residuals for BCF using nonlinear data")
plot(density(test_nl$Y - cf.pred_nl$predictions), main = "")
title(main = "Density - Residuals for CF using nonlinear data")
plot(density(test_nl$Y - Yhat_nl_lr), main = "")
title(main = "Density - Residuals for Linear regression using nonlinear data")
```
...
hist(tauhat_lin, col = rgb(1,0,0, alpha = 0.7), ylim = c(0, 2800), main = "Overlapping Histogram - tau and predicted tau - BCF- Linear data", xlab = "tau/predicted tau")
hist(tau, col = rgb(0,0,1, alpha = 0.7), add = T)

#Overlapping histograms of tau and estimated tau using BCF - non linear data
hist(tauhat_nl, col = rgb(1,0,0, alpha = 0.7), ylim = c(0, 2800), xlim = c(5, 25), main = "Overlapping Histogram - tau and predicted tau - BCF- Nonlinear data", xlab = "tau/predicted tau")
hist(tau, col = rgb(0,0,1, alpha = 0.7), add = T)
```
