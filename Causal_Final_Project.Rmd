...
Y0_nl <- 50 + (0.08*(X_mat$beta0) + runif(n, 0.02, 0.2)*(X_mat$Weight^3) + log(seq(4, 13.99, 0.01))*(X_mat$Age^6) + runif(n, 2, 4)*exp(X_mat$Size_Tumor) + (sin(X_mat$HBP)^5)/0.05 + X_mat$Relapse) *(10^-9) + 10*X_mat$Gender +rnorm(n,0,1)

Y1_nl <- 50 + (0.08*(X_mat$beta0) + runif(n, 0.02, 0.2)*(X_mat$Weight^3) + log(seq(4, 13.99, 0.01))*(X_mat$Age^6) + runif(n, 2, 4)*exp(X_mat$Size_Tumor) + (sin(X_mat$HBP)^5)/0.05 + X_mat$Relapse) *(10^-9) + 10*X_mat$Gender +rnorm(n,0,1)+ tau
...
plot(fit_lin$fitted.values, fit_lin$residuals)
abline(h = 0, col="red")
```
...
hist(tauhat_nl, col = rgb(1,0,0, alpha = 0.7), ylim = c(0, 6000), xlim = c(5, 25))
hist(tau, col = rgb(0,0,1, alpha = 0.7), add = T)
```
...
```{r}
plot(density(yhat_lin_bcf - researcher_lin$Y))
plot(density(cf.pred_lin$predictions - test_lin$Y))
plot(density(Yhat_lin_lr - test_lin$Y))

```

```{r}
plot(density(yhat_nl_bcf - researcher_nl$Y))
plot(density(cf.pred_nl$predictions - train_nl$Y))
plot(density(Yhat_nl_lr - test_nl$Y))
